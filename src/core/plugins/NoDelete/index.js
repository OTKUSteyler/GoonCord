...
function patchDispatcher(): PatchCleanupFn {
  return before("dispatch", FluxDispatcher, (args: any[]) => {
    try {
      ...
      // If this deletion was triggered by us, allow it through
      if (deleteable.includes(event.id)) {
        deleteable.splice(deleteable.indexOf(event.id), 1);
        return;
      }

      // Mark this message id so we don't loop
      deleteable.push(event.id);

      // Replace MESSAGE_DELETE with a fake automod edit-failed event
      // so the message stays visible but shows as "[deleted]"
      args[0] = {
